public class ValidacaoCpf {
     ...
     */
    private static final String REGEXCPF = "^([0-9][0-9][0-9]).([0-9][0-9][0-9]).([0-9][0-9][0-9])-([0-9][0-9])$";


    /*
     * Este trecho de código realiza a validação de um CPF, verificando se o mesmo está no formato padrão utilizado ou não;
     */
    public static void validacao(String cpf) throws ExceptionFormato {
        // Verifica se o CPF foi preenchido na hora da captura dos dados ou se o campo está vazio;
        if ((cpf == null) || cpf.isBlank()) {
            throw new ExceptionFormato("CPF vazio ou inexistente.");
        }

        /*
         * Verifica se o CPF está no formato estipulado;
         * .matches(): verifica se a string (neste caso) segue 100% a regra estipulada no REGEXCPF; 
        */

        if (!(cpf).matches(REGEXCPF)) {
            throw new ExceptionFormato("CPF Invalido.");
        }
    }
}
